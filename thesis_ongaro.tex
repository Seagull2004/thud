...
\course{Informatica}
\title{Studio del monitoraggio dello stato di pulizia dell’ottica di una telecamera industriale}
...
Per uniformare la dimensione apparente dei \textit{plate} nelle immagini provenienti da telecamere diverse, viene applicata un'operazione di ridimensionamento (\textit{resize}) all'output di ciascuna telecamera. Ad ogni telecamera sono associati due parametri, \textit{resizeWidth} e \textit{resizeHeight}, che definiscono la risoluzione finale dell'immagine. Le immagini acquisite hanno inizialmente una risoluzione fissa di 640x480 pixel. Tuttavia, poiché la distanza tra la telecamera e il \textit{plate} può variare, la dimensione del \textit{plate} all'interno dell'immagine potrebbe risultare differente. Per compensare questo effetto, le immagini vengono ridimensionate utilizzando valori specifici di \textit{resizeWidth} e \textit{resizeHeight}. Ad esempio, una telecamera posizionata molto vicino al \textit{plate} potrebbe utilizzare una risoluzione finale pari a 480x360 pixel. In questo modo, nonostante le immagini prodotte dalle diverse telecamere possano avere risoluzioni differenti, il \textit{plate} viene rappresentato con una dimensione pressoché costante, pari a circa 15x15 pixel.

\section{Generazione del dataset}

Le immagini utilizzate nello studio sperimentale sono state acquisite da diverse telecamere e rappresentano un \textit{plate} osservato in condizioni ottimali, ovvero in assenza di sporco, condensa o altri fenomeni in grado di degradare la qualità dell'immagine. Poiché non erano disponibili campioni reali di obiettivi progressivamente sporchi, è stato necessario costruire un dataset sintetico in grado di simulare tale condizione. A questo scopo, per ogni immagine originale sono state generate diverse versioni alternative applicando un filtro gaussiano con intensità crescente. Tale operazione consente di riprodurre una progressiva perdita di dettaglio simile a quella che si verifica quando l'obiettivo della telecamera risulta sporco o parzialmente ostruito.

Il filtro gaussiano effettua una convoluzione dell'immagine mediante una matrice (\textit{kernel}) i cui coefficienti seguono una distribuzione gaussiana bidimensionale. Ogni pixel dell'immagine viene quindi sostituito da una media pesata dei pixel circostanti, attribuendo un peso maggiore ai pixel più vicini al centro del kernel e un peso minore a quelli più distanti. All'aumentare della dimensione del kernel, l'effetto di sfocatura diventa progressivamente più marcato, causando una riduzione delle componenti ad alta frequenza e dei dettagli presenti nell'immagine. Nel dataset generato sono stati utilizzati kernel gaussiani di dimensione crescente, da 1x1 fino a 29x29. La dimensione massima è stata scelta sperimentalmente poiché, oltre tale valore, il codice riportato sul \textit{plate} risultava completamente illeggibile dal programma di riconoscimento. Questo approccio ha permesso di ottenere, a partire da ciascuna immagine originale, una sequenza di campioni caratterizzati da livelli di degrado progressivamente maggiori, fornendo un dataset controllato e riproducibile per l'analisi delle prestazioni delle varie metriche.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{./img/confronto-livelli-di-blur.png}
\caption{Esempio di degradazione progressiva dell'immagine mediante filtro gaussiano. Da sinistra a destra e dall'alto verso il basso viene mostrata la stessa immagine elaborata con kernel gaussiani di dimensione crescente, da 1x1 fino a 29x29.}
\end{figure}
