...
Le metriche che sono state definite offrono un indice numerico che può venire usato per determinare il livello di \textit{blur} di una immagine, ma per stabilere se una immagine sia sfocata o meno bisogna stabilire un limite (\textit{threshold}) al di sopra del quale si può dire con buona approssimazione che l'immagine sia sfocata. Ogni metrica avrà una soglia diversa data la loro natura ma sicuramente sarà un valore $t$ compreso nell'intervallo $[0,1]$. C'è la necessità di capire quindi per una certa immagine, quanto questa possa venire sfocata prima che il riconoscitore dei \textit{plate} dia un errore a causa di uno scarso livello di affidabilità, e sarà proprio questo il limite che rappresenterà il \textit{threshold} $t$. Supponiamo di avere un certo insieme di immagini $I_1,I_2,\dots,I_n$ raffiguranti un \textit{plate} ben visibile, questo insieme rappresenterà il \textit{dataset} da utilizzare per determinare in maniera empirica il valore di $t$. Sia $I_i$ una certa immagine di questo insieme mentre $I^1_i,I^2_i,I^3_i,\dots$ le versioni sfocate con un kernel via via sempre maggiore. Il riconoscitore sarà capace di individuare il codice contenuto nel \textit{plate} fino ad una certa versione $m$, mentre da $m+1$ in poi l'esito del riconoscitore sarà negativo a causa di una scarsa affidabilità data dalla sfocatura eccessiva. Per quella singola immagine otterremo un valore per $t_i$ che sarà il risultato del calcolo del \textit{blur value} $\alpha$ calcolato con la metrica scelta per l'immagine $I_i^m$.

Una volta ottenuti i valori $t_1,t_2,t_3,\dots,t_n$ derivanti dalle varie immagini si può andare a definire un valore di limite $t$ che permetta di stabilire se una nuova immagine $J$ sia sfocata o no. Un buon modo per determinare $t$ è semplcemente dato dalla media dei risultati ottenuti

\begin{equation}
  t =  \frac{1}{n} \sum_{i=1}^{n} t_i
\end{equation}

ma ovviamente è possibile utilizzare diverse altre funzioni come quella di massimo in modo da ridurre al minimo il numero di falsi positivi, ossia immagini nitide che vengono classificate come sfocate. Si può anche fare uso della funzione di minimo, in questo caso ci assicuriamo maggiori probabilità di riconoscere tutte le immagini sfocate a costo di introdurre più falso positivo.

In questo modo possiamo andare a definire dei $t$ diversi per ogni metrica, che possiamo chiamare $t_{fourier}$, $t_{skew}$, $t_{laplacian}$ e $t_{canny}$.
